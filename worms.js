...
let terrainMask, terrainSurface, selectedWeapon = 'bazooka';
...
function resetGame() {
  generateTerrain(); capybaras = [createCapybara(0, 'Luma', 130), createCapybara(0, 'Nino', 330), createCapybara(0, 'Bia', 520), createCapybara(1, 'Zeca', 700), createCapybara(1, 'Mila', 910), createCapybara(1, 'Téo', 1080)];
...
function nextTurn() {
  ...
  if (winner) { gameOver = true; turnLocked = true; messageEl.textContent = `🏆 Time ${winner.name} venceu a batalha!`; return; }
...
function drawCapybaras() {
  for (const capy of capybaras) { if (!capy.alive) continue; const active = capy === currentCapybara() && !turnLocked, bob = Math.sin(capy.phase) * 1.2, color = teams[capy.team].color; ctx.save(); ctx.translate(capy.x, capy.y + bob); ctx.scale(capy.facing, 1);
    ctx.fillStyle = capy.frozen ? '#b7f4ff' : '#9a6845'; ctx.strokeStyle = active ? '#fff59b' : color; ctx.lineWidth = active ? 4 : 2; ctx.beginPath(); ctx.ellipse(0, 0, 18, 12, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.beginPath(); ctx.arc(14, -6, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.beginPath(); ctx.arc(11, -15, 4, 0, Math.PI * 2); ctx.arc(18, -14, 4, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(18, -8, 2.7, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#25303a'; ctx.beginPath(); ctx.arc(19, -8, 1.3, 0, Math.PI * 2); ctx.arc(23, -3, 1.6, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#68452f'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(-10, 8); ctx.lineTo(-11, 16); ctx.moveTo(7, 8); ctx.lineTo(8, 16); ctx.stroke(); ctx.restore();
    ctx.fillStyle = 'rgba(5,18,28,.78)'; ctx.fillRect(capy.x - 20, capy.y - 37, 40, 5); ctx.fillStyle = capy.health > 45 ? '#70ed87' : '#ff6962'; ctx.fillRect(capy.x - 20, capy.y - 37, 40 * capy.health / 100, 5); ctx.fillStyle = '#fff'; ctx.textAlign = 'center'; ctx.font = '900 11px sans-serif'; ctx.fillText(capy.name, capy.x, capy.y - 44);
...
function touchControl(action, active) { const codes = { left: 'KeyA', right: 'KeyD', 'aim-up': 'ArrowUp', 'aim-down': 'ArrowDown' }; if (codes[action]) return active ? keys.add(codes[action]) : keys.delete(codes[action]); if (action === 'jump' && active) jump(); if (action === 'weapon' && active) cycleWeapon(); if (action === 'fire') { if (active && !turnLocked && !projectile && !gameOver) charging = true; else if (!active) shoot(); } if (action === 'skip' && active) skipTurn(); }
